...
function draw(ctx,els,bg,car,logo,bgC,ov,cP,lP){
  ...
  els.forEach(el=>{
    ...
    const txt=decodeHtml(el.text);
    const hwRaw=el.highlightWord?decodeHtml(el.highlightWord).trim():"";
    // SERVICES (multi-line per-line highlight)
    if(el.id==="services"){
      txt.split("\n").forEach((l,i)=>{const h=el.highlightLines?.includes(i);ctx.font=`${h?"700":el.fontWeight} ${el.fontSize}px "${el.fontFamily}",sans-serif`;ctx.fillStyle=h?(el.highlightColor||"#00BCD4"):el.color;ctx.fillText(l,el.x,el.y+i*(el.fontSize+6));});
      ctx.restore();return;
    }
    // PILL (duration)
    if(el.pillStyle){
      const tw=ctx.measureText(txt).width,ph=el.fontSize+16,pw=tw+40,rx=el.textAlign==="center"?el.x-pw/2:el.x;
      ctx.fillStyle=el.bgColor||"#1a1a2e";ctx.beginPath();const r=6;
      ctx.moveTo(rx+r,el.y-ph/2);ctx.lineTo(rx+pw-r,el.y-ph/2);ctx.quadraticCurveTo(rx+pw,el.y-ph/2,rx+pw,el.y-ph/2+r);ctx.lineTo(rx+pw,el.y+ph/2-r);ctx.quadraticCurveTo(rx+pw,el.y+ph/2,rx+pw-r,el.y+ph/2);ctx.lineTo(rx+r,el.y+ph/2);ctx.quadraticCurveTo(rx,el.y+ph/2,rx,el.y+ph/2-r);ctx.lineTo(rx,el.y-ph/2+r);ctx.quadraticCurveTo(rx,el.y-ph/2,rx+r,el.y-ph/2);ctx.closePath();ctx.fill();
      if(hwRaw){
        const parts=txt.split("–").map(s=>s.trim());
        if(parts.length===2){const p1=parts[0]+" – ",sx=el.textAlign==="center"?el.x-tw/2:el.x;ctx.textAlign="left";ctx.fillStyle=el.color;ctx.fillText(p1,sx,el.y);ctx.fillStyle=el.highlightColor||"#00BCD4";ctx.fillText(parts[1],sx+ctx.measureText(p1).width,el.y);}
        else{ctx.fillStyle=el.color;ctx.fillText(txt,el.x,el.y);}
      } else {ctx.fillStyle=el.color;ctx.fillText(txt,el.x,el.y);}
      ctx.restore();return;
    }
    // GENERAL: multi-line + always-visible highlight (append if not found)
    const lines=txt.split("\n");
    const lineH=el.fontSize+6;
    const baseY=el.y-(lines.length-1)*lineH/2;
    if(hwRaw){
      const nHw=NORM(hwRaw);
      const foundInWhole=NORM(txt).indexOf(nHw)>=0;
      lines.forEach((line,i)=>{
        const ly=baseY+i*lineH;
        const idxI=NORM(line).indexOf(nHw);
        let before,hl,after;
        if(idxI>=0){
          before=line.substring(0,idxI);
          hl=line.substring(idxI,idxI+hwRaw.length);
          after=line.substring(idxI+hwRaw.length);
        } else if(!foundInWhole && i===lines.length-1){
          // append highlight to last line
          before=line+(line?" ":"");
          hl=hwRaw;
          after="";
        } else {
          ctx.fillStyle=el.color;ctx.fillText(line,el.x,ly);return;
        }
        const bw=ctx.measureText(before).width,hlw=ctx.measureText(hl).width,aw=ctx.measureText(after).width;
        const fw=bw+hlw+aw;
        const sx=el.textAlign==="center"?el.x-fw/2:(el.textAlign==="right"?el.x-fw:el.x);
        const savedAlign=ctx.textAlign;
        ctx.textAlign="left";
        ctx.fillStyle=el.color;if(before)ctx.fillText(before,sx,ly);
        ctx.fillStyle=el.highlightColor||"#00BCD4";ctx.fillText(hl,sx+bw,ly);
        ctx.fillStyle=el.color;if(after)ctx.fillText(after,sx+bw+hlw,ly);
        ctx.textAlign=savedAlign;
      });
    } else {
      lines.forEach((line,i)=>{ctx.fillStyle=el.color;ctx.fillText(line,el.x,baseY+i*lineH);});
    }
    ctx.restore();
  });
...
export default function App(){
  ...
  const snap=useCallback(()=>{if(isUndo.current)return;const s=JSON.stringify({elements,offers:offers.map(o=>({...o,carImg:null,carThumb:o.carThumb})),carPos,logoPos,bgColor,overlayOpacity});const h=histRef.current;if(!h.length||h[h.length-1]!==s){h.push(s);if(h.length>100)h.shift();}},[elements,offers,carPos,logoPos,bgColor,overlayOpacity]);
  const snapT=useRef(null);
  useEffect(()=>{if(snapT.current)clearTimeout(snapT.current);snapT.current=setTimeout(snap,300);},[snap]);
  // Initial snapshot once after mount
  ...
  useEffect(()=>{const h=e=>{const tag=(e.target&&e.target.tagName)||"";const isField=tag==="INPUT"||tag==="TEXTAREA"||(e.target&&e.target.isContentEditable);if((e.ctrlKey||e.metaKey)&&e.key==="z"&&!e.shiftKey){if(isField)return;e.preventDefault();undo();}};window.addEventListener("keydown",h);return()=>window.removeEventListener("keydown",h);},[undo]);
  ...
  const uploadFont=async(e)=>{
    ...
    for(const file of files){
      try{
        ...
        setCustomFonts(p=>p.includes(name)?p:[...p,name]);
      }catch(err){console.error("Errore caricamento font:",file.name,err);alert("Impossibile caricare il font "+file.name+": "+err.message);}
    }
    e.target.value="";
  };
  const removeCustomFont=(name)=>{setCustomFonts(p=>p.filter(n=>n!==name));setElements(p=>p.map(el=>el.fontFamily===name?{...el,fontFamily:"Montserrat"}:el));};

  useEffect(()=>{const l=document.createElement("link");l.href=`https://fonts.googleapis.com/css2?${FONTS.map(f=>`family=${f.replace(/ /g,"+")}:wght@100;200;300;400;500;600;700;800;900`).join("&")}&display=swap`;l.rel="stylesheet";document.head.appendChild(l);document.fonts.ready.then(()=>setFontsLoaded(true));},[]);
